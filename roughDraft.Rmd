...
ggplot(data=tuning_curves1, aes(x=angle_bins, y=firing_rate, color=tetrode)) +
    facet_wrap( ~ cellname, ncol=10, scale='free_y') +
    geom_line() +
...
thalamus_cellnames2 = c('T5C5','T5C7')
...
thalamus_cellnames3 = c('T8C2','T8C5','T8C6','T9C10','T9C11','T9C12','T9C13')
...
cortex_cellnames = unique(cortex_cells[!(cortex_cells$cellname=='T1C1'),]$cellname)
cortex_cells = cortex_cells[!(cortex_cells$cellname=='T1C1'),]
...
shifted_cortex = unique(cortex_cells$cellname)
...
For most cells both models proved effective. The only two exceptions are cell T5C3 and T7C4.  
...
for(smoothing_window in smoothing_windows[4:6]){
...
# colnames(predictions) = as.character(c(5,10,smoothing_windows))
```
